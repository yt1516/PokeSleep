...
def get_sound(sound):
    sound_log = pd.DataFrame(columns = ['Datetime','Trigger bool'])
    values_list = sound.col_values(1)
    for i in range(len(values_list)):
        if values_list[i]:
            t = values_list[i]
            t_std = datetime.strptime(t, "%Y-%m-%d_%H.%M.%S")
            sound_log = sound_log.append({'Datetime':t_std, 'Trigger bool':1},ignore_index=True)
            sound_log['Datetime'] = pd.to_datetime(sound_log['Datetime'])
    sound_log = sound_log.drop_duplicates(keep='first')
    sound_log = sound_log.set_index(sound_log['Datetime'])
    return sound_log
...
def get_outside_temp(outside):
    out_temp = pd.DataFrame(columns = ['Datetime','local temperature','local humidity','atmospheric pressure','wind speed','cloud'
                                      'weather status'])
    ...
    out_cloud = outside.col_values(6)
    out_stat = outside.col_values(7)
    for i in range(len(out_time)):
        if out_time[i]:
            ...
            out_temp = out_temp.append({'Datetime':t_std,'local temperature':out_tempts[i],
                                        ...
                                        'wind speed':out_wind[i],'cloud':out_cloud[i],'weather status':out_stat[i]},ignore_index=True)
